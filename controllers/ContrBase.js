...
exports.ajout = async (req, res, table, data, passAdmin) => {
    ...
    if (table.getTableName() == "Utilisateurs") {
        ...
        try {
            ...
            const token = jwt.sign({id : prevData.id , email : prevData.email} , "secret", {expiresIn : '3m'})
            return passAdmin ? res.status(200).json({ serverMsgSuccess: `Inscription réuissie`, passAdmin }) : res.status(200).json({token , badge : prevData.badge , status : prevData.status})
        ...
        try {
            ...
            if (dataLivre.exemplaire !== 0) {
                ...
                const newNotification = Notification.build({ UtilisateurId: utilisateurConcerne.id, NotificationType: "Reservation", serverSuccessMsg: `${utilisateurConcerne.nom} ${utilisateurConcerne.prenom} a fait une réservation pour le livre ${dataLivre.titre}.` })
                await newNotification.save()

                return res.status(200).json({ serverSuccessMsg: `Réservation réussie pour le livre ${dataLivre.titre}` })

            } else { return res.status(200).json({ serverErrorMsg: `Ce livre n'est pas encore disponible pour le moment!` }) }
...
exports.voirTout = async (req, res, table, badge ) => {
    ...
    try {
        ...
        return res.status(200).json({ serverMsgSuccess: `${data.length} ${table.getTableName()}`, data })
...
exports.recherche = async (req, res, table) => {
    ...
    if (table.getTableName() == 'Livres') {
        ...
        try {
            ...
            const {count , rows} = await Livre.findAndCountAll({include : Categorie , where : whereClause, limit : pageSiz , offset:(activePage - 1) * pageSiz})

            return res.status(200).json({currentPage : activePage , data : rows , totalPage : Math.ceil(count/pageSiz) , count })
        ...
        try {
            ...
            return dataReservation.length !== 0 ? res.status(200).json({ dataReservation }) : res.status(200).json({ serverErrorMsg: `Aucun resultat!` })

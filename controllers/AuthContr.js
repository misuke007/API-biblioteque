...
exports.enregistrementMembre = async(req, res) => {
    ...
    if(utilisateur){
        ...
        return res.status(200).json({serverMsgError : 'Cet email est déjà utilisé!'})
...
exports.verificationCarte = async(req, res) => {
    ...
    console.log(numero  , cvv  , date_expiration)
    ...
    try{
        ...
        if(carte){
            ...
            }else{return res.status(200).json({valide : false , serverMsgError : "Votre carte est expirée"})}
    

        }else{return res.status(200).json({valide : false , serverMsgError : "Carte invalide"})}
...
exports.validationPaiement = async(req, res) => {
    ...
    try{
        ...
        if(mot_de_passe == cbData.mot_de_passe){
            ...
            return res.status(200).json({ valide : false , serverMsgError : `Vous n'avez pas assez d'argent pour payer le droit! Veuillez consulter votre solde`})
            ...
            return res.status(200).json({ valide : true ,  server_msg_success : `Votre inscription est validé , connectez-vous maintenant  pour profiter de l'application `})
           ...
           return res.status(200).json({valide  : false , serverMsgError : "Mot de passe incorrecte!"})
        }
...
exports.login = async(req, res) => {
    ...
    try{
        ...
        if(!utilisateur) return res.status(200).json({serverMsgError : "Votre e-mail est invalide!"})
        const verificationMdp = bcrypt.compareSync(mot_de_passe , utilisateur.mot_de_passe)
        if(!verificationMdp) return res.status(200).json({serverMsgError : "Mot de passe invalide!"})

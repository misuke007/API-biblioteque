...
exports.ajoutReservation = (req, res) => {
  ...
  ajout(req, res, Reservation, {
    date_recuperation: dateDexpiration,
    LivreId,
    UtilisateurId,
  });
};
...
exports.validationEmprunt = async (req, res) => {
  ...
  try {
    const dataReservation = await Reservation.findOne({
      include: [Utilisateur, Livre],
      where: { UtilisateurId, LivreId },
    });
    ...
    await Reservation.destroy({
      where: { LivreId: dataReservation.LivreId, UtilisateurId: UtilisateurId },
    });
    return res.status(200).json({
      message: `Emprunt du livre ${dataReservation.Livre.titre} validé!`,
    });
  } catch (error) {
